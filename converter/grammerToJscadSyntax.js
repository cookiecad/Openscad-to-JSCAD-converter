...
function translateSEQ(def) {
  ...
  const close = def.members[def.members.length-1].type === "STRING" ? def.members[def.members.length-1].value : '';
  return {
      open,
      close,
...
function getSymbols(members) {
  ...
  return symbols.map(symbol => ({childIndex: getChildIndex(members, symbol.name), name: symbol.name, optional: false}));
}

function getChildIndex(members, symbolName) {
  return members.findIndex(member => member.type === 'SYMBOL' && member.name === symbolName);
}

let file = String.raw`..\tree-sitter-openscad\src\grammar.json`
translate(file, function(err, result) {
  if (err) {
    console.error(err);
    return;
  }
  fs.writeFileSync('jscadSyntaxFromGrammar.json', JSON.stringify(result, null, 2));
